...
from src.api.error_messages import translate_server_message # noqa: E402
from src.api.services import ServicesAPI # noqa: E402
from src.dialogs.app_dialogs import alert, confirm # noqa: E402
from src.icons import icon_search, logo_pixmap, icon_sun, icon_moon, icon_refresh # noqa: E402
...
class AppWidget(QWidget):
    def __init__(self, parent: QWidget | None = None):
        ...
        self._pending_launch_transport: object | None = None
        self._toast_label: QLabel | None = None
        self._toast_hide_timer: QTimer | None = None
        self._services_refresh_timer: QTimer | None = None
        self._services_refresh_silent = False
        self._services_refresh_in_progress = False
        self._pending_last_login: str = ""
        ...
        self._init_toast()
        self._init_services_refresh_timer()
    ...
    def _connect_signals(self) -> None:
        ...
        self._services_api.action_done.connect(self._on_service_action_done)
        ...
        self.main_page.transport_requested.connect(self._on_service_transport_requested)
    ...
    def _on_language_changed(self, locale: str) -> None:
        ...
        btn_refresh = getattr(self.ui, "btnRefresh", None)
        if btn_refresh is not None:
            btn_refresh.setToolTip(self.tr("Refresh services"))
    ...
    def _init_toast(self) -> None:
        ...
        self._toast_label.setObjectName("toastNotification")
    ...
    def _init_services_refresh_timer(self) -> None:
        self._services_refresh_timer = QTimer(self)
        self._services_refresh_timer.setInterval(30_000)
        self._services_refresh_timer.timeout.connect(lambda: self._refresh_services(silent=True))

    def _init_refresh_button(self) -> None:
        btn = getattr(self.ui, "btnRefresh", None)
        if btn is None:
            return
        btn.setText("")
        btn.setCursor(Qt.CursorShape.PointingHandCursor)
        btn.setIcon(icon_refresh(color="#E5E7EB"))
        btn.setToolTip(self.tr("Refresh services"))
        btn.clicked.connect(lambda: self._refresh_services(silent=False))
    ...
    def _refresh_login_toolbar_texts(self) -> None:
        ...
        if btn:
            ...
            btn.setText("")
            btn.setIcon(icon_sun(color="FFFFFF") if is_dark else icon_moon(color="000000"))

            btn.setToolTip(
                self.tr("Switch to light theme") if is_dark
                else self.tr("Switch to dark theme")
            )
    ...
    def _refresh_search_icon(self) -> None:
        ...
        if btn is not None:
            color = "#E5E7EB"
            btn.setIcon(icon_refresh(color=color))
    ...
    def _on_login_success(self, token: str, username: str, last_login: str = "") -> None:
        ...
        self._pending_last_login = last_login.strip()
        if username:
    ...
    def _on_services_ready(self, services) -> None:
        if not self._initial_workspace_load:
            was_silent = self._services_refresh_silent
            self._services_refresh_silent = False
            self._services_refresh_in_progress = False
            self._set_refresh_button_busy(False)
            self.main_page.populate(services)
            if not was_silent:
                self._show_toast(self.tr("Services refreshed"), kind="success", timeout_ms=4500)
            return
    ...
    def _finish_initial_workspace_load(self, failed) -> None:
        ...
        self._initial_workspace_load = False
        self.show_main()
        self._start_services_auto_refresh()
        self._show_last_login_toast()
    ...
    def _on_services_error(self, msg) -> None:
        ...
        if self._initial_workspace_load:
            ...
            self.show_loading_error(msg)
        if was_silent:
            log.warning("Silent services refresh failed: %s", msg)
            return
        alert(self, self.tr("Loading error"), msg, kind="danger")

    def _on_connect_error(self, msg) -> None:
        log.warning("Connect error: %s", msg)
        alert(self, self.tr("Warning"), translate_server_message(msg), kind="warning")

    def _on_login_failed(self, msg) -> None:
        log.warning("Login failed: %s", msg)
        self._stop_services_auto_refresh()
        self._initial_workspace_load = False
        self.ui.stackedWidget.setCurrentWidget(self.ui.pageLogin)
    ...
    def _on_service_card_clicked(self, svc) -> None:
        self._start_service_connection(svc, ask_custom_message=True)

    def _on_service_transport_requested(self, svc, transport) -> None:
        self._start_service_connection(
            svc,
            transport=transport,
            ask_custom_message=True,
        )

    def _start_service_connection(
        self,
        svc,
        *,
        transport=None,
        ask_custom_message: bool = True,
    ) -> None:
        ...
        selected_transport = transport or svc.default_transport
        ...
        if ask_custom_message and not self._confirm_service_custom_message(svc):
            return

        notice = self._service_launch_status_notice(svc)
        if notice:
            self._show_toast(notice, kind="info", timeout_ms=7000)

        log.info(
            "Launch transport: service=%s transport=%s",
            svc.name,
            selected_transport.name,
        )
        self._pending_launch_svc = svc
        self._pending_launch_transport = selected_transport
    ...
    def _confirm_service_custom_message(self, svc) -> bool:
        custom_message = str(getattr(svc, "custom_message", "") or "").strip()
        if not custom_message:
            return True

        service_title = (
            getattr(svc, "visual_name", "")
            or getattr(svc, "name", "")
            or self.tr("selected service")
        )

        return confirm(
            self,
            title=self.tr("Service message"),
            text=f"{service_title}\n\n{custom_message}",
            confirm_text=self.tr("Continue"),
    ...
    def _on_service_action_requested(self, svc, action) -> None:
        ...
        self._show_toast(self._service_action_started_text(action), kind="info", timeout_ms=7000)
        self._services_api.service_action(svc.id, action)

    def _on_service_action_done(self, service_id: str, action: str) -> None:
        del service_id
        self._show_toast(self._service_action_done_text(action), kind="success", timeout_ms=7000)
        self._refresh_services(silent=True)

    def _service_action_started_text(self, action: str) -> str:
        messages = {
            "poweron": self.tr("Power on command sent"),
            "poweroff": self.tr("Power off command sent"),
            "reset": self.tr("Reset command sent"),
        }
        return messages.get(action, self.tr("Action command sent"))

    def _service_action_done_text(self, action: str) -> str:
        messages = {
            "poweron": self.tr("Power on completed"),
            "poweroff": self.tr("Power off completed"),
            "reset": self.tr("Reset completed"),
        }
        return messages.get(action, self.tr("Action completed"))
    ...
    def _on_transport_ok(self) -> None:
        ...
        self._pending_launch_transport = None
        self.show_main()

    def _on_transport_error(self, msg) -> None:
        log.warning("Transport error: %s", msg)
        svc = self._pending_launch_svc
        display_msg = self._service_not_ready_message(svc, msg)
        self._spinner.stop()
        self._pending_launch_svc = None
        self._pending_launch_transport = None
        self.show_main()
        self._show_transport_error(display_msg)

    def _on_transport_retry(self, msg) -> None:

        svc = self._pending_launch_svc
        transport = self._pending_launch_transport
        display_msg = self._service_not_ready_message(svc, msg)
        ok = confirm(
            self,
        ...
        if ok and svc:
            QTimer.singleShot(
                3000,
                lambda: self._start_service_connection(
                    svc,
                    transport=transport,
                    ask_custom_message=False,
                ),
            )
        else:
            self._spinner.stop()
    ...
    def _show_toast(self, text: str, *, kind: str = "success", timeout_ms: int = 6000) -> None:
        if not self._toast_label:
            return

        self._toast_label.setText(text)
        self._toast_label.setProperty("kind", kind)
        self._toast_label.style().unpolish(self._toast_label)
        self._toast_label.style().polish(self._toast_label)
        self._toast_label.update()
        self._position_toast()
        self._toast_label.show()
        self._toast_label.raise_()

        if self._toast_hide_timer:
            self._toast_hide_timer.start(max(1000, timeout_ms))
    ...
    def _position_toast(self) -> None:
        if not self._toast_label:
            return
        self._toast_label.setFixedWidth(min(360, max(260, self.width() - 48)))
        self._toast_label.adjustSize()
        x = max(16, self.width() - self._toast_label.width() - 24)
        y = max(16, self.height() - self._toast_label.height() - 24)
        self._toast_label.move(x, y)

    def _refresh_services(self, *, silent: bool = True) -> None:
        if (
            self._initial_workspace_load
            or not self._auth.is_authenticated
            or self._services_refresh_in_progress
        ):
            return
        self._services_refresh_in_progress = True
        self._services_refresh_silent = silent
    ...
    def _start_services_auto_refresh(self) -> None:
        if self._services_refresh_timer and not self._services_refresh_timer.isActive():
            self._services_refresh_timer.start()

    def _stop_services_auto_refresh(self) -> None:
        if self._services_refresh_timer:
            self._services_refresh_timer.stop()
        self._set_refresh_button_busy(False)
        self._services_refresh_silent = False
        self._services_refresh_in_progress = False

    def _show_last_login_toast(self) -> None:
        raw = self._pending_last_login.strip()
        self._pending_last_login = ""
        if not raw:
            return
        display = self._format_last_login(raw)
        self._show_toast(
            self.tr("Previous login: {date}").format(date=display),
            kind="info",
            timeout_ms=9000,
        )

    def _format_last_login(self, raw: str) -> str:
        try:
    ...
    def _service_status_code(self, svc) -> str:
        return str(getattr(svc, "status_code", "") or "").strip().lower()

    def _service_launch_status_notice(self, svc) -> str:
        code = self._service_status_code(svc)
        messages = {
            "off": self.tr("The machine is powered off. Power on is being started."),
            "powering_on": self.tr("The machine is powering on. Connection will be available soon."),
            "rebooting": self.tr("The machine is rebooting. Connection will be available after reboot."),
            "preparing": self.tr("The machine is being prepared. Connection will be available soon."),
            "powering_off": self.tr("The machine is powering off. Please wait until the operation finishes."),
            "maintenance": self.tr("The service is in maintenance mode. Connection is temporarily unavailable."),
            "unavailable": self.tr("The service is unavailable. Please try again later."),
        }
        return messages.get(code, "")

    def _service_not_ready_message(self, svc, fallback: str) -> str:
        code = self._service_status_code(svc)
        messages = {
            "off": self.tr("The machine is powered off. Power on is being started."),
            "powering_on": self.tr("The machine is powering on. Please try again shortly."),
            "rebooting": self.tr("The machine is rebooting. Please try again after reboot finishes."),
            "preparing": self.tr("The machine is being prepared. Please try again shortly."),
            "powering_off": self.tr("The machine is powering off. Please wait until the operation finishes."),
            "maintenance": self.tr("The service is in maintenance mode. Connection is temporarily unavailable."),
            "unavailable": self.tr("The service is unavailable. Please try again later."),
        }
        return messages.get(code, translate_server_message(fallback))
